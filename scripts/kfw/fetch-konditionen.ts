...
const INTERHYP_SOURCE_URL = 'https://www.interhyp.de/zinsen/';
...
async function fetchMarketRateInterhyp(): Promise<number | null> {
  const response = await fetch(INTERHYP_SOURCE_URL, {
    method: 'GET',
    signal: AbortSignal.timeout(30_000),
    headers: {
      'user-agent':
  ...
  const headingMatch = html.match(/<h[1-6][^>]*>[\s\S]*?Zinstabelle:\s*Effektiver\s*Jahreszins[\s\S]*?<\/h[1-6]>/i);
  if (!headingMatch || headingMatch.index === undefined) {
    return null;
  }

  const htmlAfterHeading = html.slice(headingMatch.index + headingMatch[0].length);
  const tableMatch = htmlAfterHeading.match(/<table\b[^>]*>[\s\S]*?<\/table>/i);
  if (!tableMatch) {
    return null;
  }
  ...
  const cellText = (cellHtml: string): string => {
    ...
    return normalizeWhitespace(decodeHtmlEntities(withoutComments.replace(/<[^>]+>/g, ' ')));
  };

  const parseRowCells = (rowHtml: string): string[] => {
...
async function main(): Promise<void> {
  ...
  let marketRate = await fetchMarketRateInterhyp();
  let marketSourceName = 'Interhyp';
  let marketSourceUrl = INTERHYP_SOURCE_URL;

  if (marketRate === null) {
    marketRate = await fetchMarketRateCheck24();
    marketSourceName = 'CHECK24';
  ...
  if (marketRate === null && lkg?.interhyp?.rates?.zinsbindung_10?.zins !== undefined) {
    ...
    if (Number.isFinite(lkgRate) && lkgRate > 0) {
      marketRate = lkgRate;
      const lkgSourceName = lkg.interhyp.source_name;
      const lkgSourceUrl = lkg.interhyp.source_url;
      marketSourceName = typeof lkgSourceName === 'string' && lkgSourceName ? `${lkgSourceName} (Fallback Last-known-good)` : 'Fallback Last-known-good';
      marketSourceUrl = typeof lkgSourceUrl === 'string' && lkgSourceUrl ? lkgSourceUrl : INTERHYP_SOURCE_URL;
    }
  }

  if (marketRate === null) {
    marketRate = 3.96;
    marketSourceName = 'Interhyp/Check24 (Fallback)';
    marketSourceUrl = INTERHYP_SOURCE_URL;
  }
